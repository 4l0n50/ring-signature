...
$\GG$, and $e:\GG\times\GG\to\GG_T$ is an efficiently computable and non-degenerated bilinear map. We denote by $|\GG|$ the bit-size of the elements of $\GG$.

Elements in $\GG$, are denoted implicitly as $[a]:=a \Pt$ and $\Pt_T:=e(\Pt,\Pt)$. 
The pairing operation will be written as a product $\cdot$, that is $[a] \cdot [b]=[a] [b]=e([a],[b])=[ab]_T$. Vectors and matrices are denoted in boldface. Given a matrix $\matr{T}=(t_{i,j})$, $[\matr{T}]$ is
