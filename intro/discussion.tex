...
If we use Gonzalez et al.'s techniques we would have to show that for all $[\vecb{a}']\in A',$ $[\vecb{a}']\in A_ \mu$. However, we can't do this since $A_\mu$ is unknown to the verifier. Instead, we are using features of the permutation pairing assumption that where ignored by Gonzalez et al.~and by Groth and Lu. Indeed, we use $h(A_\mu)$ as a constant-size commitment to $A_\mu$ while the permutation pairing assumption helps us to still be able to prove that $A'=A_\mu$. 
