...
\begin{align*}
&g_{\matr{A}'}(\vecb{x}')=\vecb{g}'. %\label{eq-rs-1}
\end{align*}
Perfect soundness of proof $\pi_g$ and $\pi_{\mathcal{Q}}$ implies that
\begin{align*}
&h(A')=\vecb{h}'\text{ and } A'\in \mathcal{Q}_m.%\label{eq-rs-2}
\end{align*}
Given perfect soundness of proofs $\pi_G,\pi_H$, it holds that that
\begin{align*}
&g_{\matr{A}'}(\vecb{x}')=g_{\matr{A}_\mu}(\vecb{x}_\mu)%\label{eq-rs-3}
\\
&h(A')=h(A_\mu), %\label{eq-rs-4}.
\end{align*}
and hence $\matr{A}'\equiv_\matr{U} \matr{A}$. %there exists some permutation matrix $\matr{P}\in\bits^{m\times m}$ and $\vecb{\delta}\in\Z_q^m$ such that $\matr{A}' = \matr{A}\matr{P}+\vecb{u}\vecb{\delta}^\top$.
